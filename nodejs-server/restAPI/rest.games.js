...
class RestGames extends rest_base_1.RestBase {
    constructor(handlerSettings) {
        super(handlerSettings);
        this.lobby = (request, response, next) => {
            this.mongoDB.Game
                .find({ status: 'P' })
                .sort({ createdTime: -1 })
                .limit(35)
        ...
        this.history = (request, response, next) => {
        ...
        this.joinGame = (request, response, next) => {
            let thisObj = this;
        ...
        this.removeGame = (request, response, next) => {
            let thisObj = this;
            if (request.params.id == null) {
                console.error('RestGames.joinGame: Invalid parameters');
                response.send(400, { 'msg': 'Invalid parameters' } || {});
    ...
    updateExcessiveLobbyGames(games, mantainLimit) {
        let i = 0;
